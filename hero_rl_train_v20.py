"""Train V20: BC + fine-tune in the extended action space (v3).

Rerun of V19 against a corrected opponent. V19 trained in an env that
deferred the heuristic opponent's sacrifice and discard choices with nothing
to answer them, so those effects were silently deleted and the opponent was
not the one the v2 baselines were measured against.

V19 still read 50.0% +/- 4.0pp held-out afterwards, against V16's 48.0% - a
+2.0pp difference with a 95% CI of [-3.7, +7.7], i.e. no measurable gain.
This run is the clean test of whether sacrifice/discard targeting helps at
all, with the opponent fixed. V16's
...
class EvalCallback(BaseCallback):
    ...
    def _on_step(self):
        if self.n_calls % self.eval_freq == 0:
            self.model.save(os.path.join(self.save_dir, f"v20_{self.n_calls}"))
            wr = evaluate(lambda e, o: self.model.predict(
                o, action_masks=e.action_masks(), deterministic=True)[0])
            flag = ""
            if wr > self.best:
                self.best = wr
                self.model.save(os.path.join(self.save_dir, "v20_best"))
                flag = "  <- best"
...
model.save(os.path.join(CHECKPOINT_DIR, "v20_bc"))
...
model.save(os.path.join(CHECKPOINT_DIR, "v20_final"))
