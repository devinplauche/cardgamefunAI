"""Train V11: no Fire Gem option, end-game reward, 2M steps, 128 hidden."""
import os
...
class CheckpointCallback(BaseCallback):
    ...
    def _on_step(self):
        if self.n_calls % self.save_freq == 0:
            path = os.path.join(self.save_dir, f"v11_{self.n_calls}")
    ...
    def _on_step(self):
        if self.n_calls % self.eval_freq == 0:
            ...
            for _ in range(self.n_episodes):
                env = HeroRealmsEnv(cards, opponent_profile="random", allow_fire_gem=False)
                obs, _ = env.reset()
                done = False
                while not done:
...
print(f"Training V11 for {TOTAL_TIMESTEPS} timesteps...")
print("  No Fire Gem option (action 5 = pass)")
print("  Network: 128x128, End-game reward, Random opponents")
print()
...
final_path = os.path.join(CHECKPOINT_DIR, "v11_final")
